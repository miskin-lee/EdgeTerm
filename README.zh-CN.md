...
**Session → Duplicate Tab**（标签右键菜单里也有）再开一个同 profile 的会话。SSH / SFTP 会直接开在这个标签已经在用的那条连接上——只是这条连接上的又一个通道，不握手、不需要再登录一次。开了 MFA 的服务器因此只在建立连接时验证一次，而不是每开一个标签都要输一次验证码；**Split Right** / **Split Down** 同样复用。共用一条连接的标签彼此独立：各有各的 Shell，关掉其中一个不影响其它；但这条连接断了它们会一起断，而连接已经没了时则照常重新拨号。
