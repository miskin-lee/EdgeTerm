...
- **时间戳 + 行号侧栏** —— WindTerm 最有辨识度的特性，每一行输出都带 `[HH:MM:SS]` 与累计行号，光标行高亮。可在 `Mode` 菜单切换四种显示模式。
- **Explorer**（左上）：当前已打开的会话列表
- **Filer**（左下）：文件浏览器。SSH 会话下自动切到 SFTP，可上传 / 下载 / 新建目录 / 删除；其他会话下浏览本地文件系统
- **Session**（右上）：保存的连接配置，按分组折叠，双击连接
- **Outline**（右下）：自动记录你执行过的命令及其行号，点击跳回
- **Sender**（底部）：批量发送。支持文本 / 十六进制、按行 / 按字符、重复次数、发送间隔、目标为当前会话或全部会话
- **Log**（底部，与 Sender 同区）：会话连接、断开、错误事件
...
- 保存的配置**不含密码和密钥口令**（配置文件权限为 0600，但仍是明文 JSON，所以密钥类信息只保留在内存中）。每次连接需要重新输入，或改用 ssh-agent / 公钥认证。
